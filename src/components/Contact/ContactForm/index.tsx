...
export const ContactForm = () => {
  ...
  const sendEmail: SubmitHandler<TInputs> = (formData: any) => {
    try {
      emailjs.send('service_6d7lceo', 'template_nzyz4ph', formData, 'R4L-T1FPTvIgE6xIx')
      reset()
    } catch (e) {
      console.log(e)
    }
  ...
  return (
    <form onSubmit={handleSubmit(sendEmail)}>
      ...
      {(errors?.name?.type === 'required' || errors?.email?.type === 'required') && (
